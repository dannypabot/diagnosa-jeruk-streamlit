...
def get_gejala():
    ...
    conn.close()
    return {row["kode_gejala"]: row["deskripsi"] for row in rows}

# Ambil data penyakit dan basis pengetahuan
def get_data_penyakit():
    ...
    for row in basis_rows:
        data[row["kode_penyakit"]]["gejala"][row["kode_gejala"]] = row["nilai_probabilitas"]
...
def simpan_diagnosa(gejala, hasil):
    conn = get_connection()
    cursor = conn.cursor()
    query = "INSERT INTO hasil_diagnosa (gejala_terpilih, hasil, tanggal_diagnosa) VALUES (%s, %s, %s)"
